...
class AuthService {
  ...
  // ==================== Multi-Step Face Embedding (Fixed) ====================
    ...
    try {
      ...
      for (int i = 0; i < imagePaths.length; i++) {
        ...
        try {
          ...
          // แก้ไข: ใช้ generateEmbedding แทน extractFaceEmbedding
          final embedding = await faceService.generateEmbedding(imagePath);
          final quality = _calculateAdvancedEmbeddingQuality(embedding);
          ...
          print('⚠️ Failed to process image ${i + 1}: $e');
        }
      }
  ...
  Future<Map<String, dynamic>> verifyFaceFromMultipleImages(
    ...
    try {
      ...
      for (int i = 0; i < imagePaths.length; i++) {
        ...
        try {
          // แก้ไข: ใช้ generateEmbedding แทน extractFaceEmbedding
          final capturedEmbedding = await faceService.generateEmbedding(imagePath);
          final quality = _calculateAdvancedEmbeddingQuality(capturedEmbedding);
